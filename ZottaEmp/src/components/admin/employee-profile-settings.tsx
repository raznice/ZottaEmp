// src/components/admin/employee-profile-settings.tsx
'use client';
...
import { initiateAdminProfileChange, confirmAdminProfileChange } from '@/lib/actions'; // Assuming these actions are general enough
...
type EmployeeProfileSettingsDictionary = {
    ...
    AdminCredsUpdatedToastMessage: string; // Consider renaming if for general employee
    VerificationFailToastTitle: string;
    CouldNotConfirmProfileChangeError: string;
    AdminProfileSettingsTitle: string; // Consider renaming to EmployeeProfileSettingsTitle
    AdminProfileSettingsDescription: string; // Consider renaming
...
interface EmployeeProfileSettingsProps {
    dictionary: EmployeeProfileSettingsDictionary;
    locale: Locale;
}

// Schema definitions are outside the component
// They explicitly take `theDictionary` and `theLocale` as parameters.
const getProfileChangeSchema = (theDictionary: EmployeeProfileSettingsDictionary, theLocale: Locale) => z.object({
...
const getTokenSchema = (theDictionary: EmployeeProfileSettingsDictionary, theLocale: Locale) => z.object({
    token: z.string().min(1, theDictionary.TokenLabel + (theLocale === 'it' ? ' è obbligatorio/a' : ' is required')),
});


export function EmployeeProfileSettings({ dictionary, locale }: EmployeeProfileSettingsProps) {
    const { user, logout } = useAuth(); // Assuming this hook is relevant for employees too
    ...
    const handleInitiateChange = async (values: z.infer<typeof profileChangeSchemaInstance>) => {
        if (!user) { // Removed role check, assuming employee might change their own profile
        ...
        setIsLoading(true);
        try {
            // Using initiateAdminProfileChange, but it might need to be a more general `initiateProfileChange`
            // if this component is for employees. For now, keeping it as is.
    ...
    const handleConfirmChange = async (values: z.infer<typeof tokenSchemaInstance>) => {
        ...
        setIsLoading(true);
        try {
            // Using confirmAdminProfileChange, adjust if needed for employee context
            const result = await confirmAdminProfileChange(user.id, values.token);
            if (result.success) {
                toast({
                    title: dictionary.SuccessToastTitle,
                    description: dictionary.AdminCredsUpdatedToastMessage, // May need new key for employee
